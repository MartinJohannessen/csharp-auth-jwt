...
namespace exercise.wwwapi.Endpoints
{
    public class PostEndpoint
    {
        ...
        public static void ConfigurePostEndpoint(this WebApplication app)
        {
            ...
            var cars = app.MapGroup("posts");
            cars.MapGet("/", GetAll);
            cars.MapPost("/", AddPost).AddEndpointFilter(async (invocationContext, next) =>
            {
                var car = invocationContext.GetArgument<PostPostRequest>(1);
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public static async Task<IResult> Update(IRepository<Car> repository, int id, CarPatchRequest model)
        {
            var entity = await repository.GetById(id);
            if (entity == null)
            {
        ...
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        public static async Task<IResult> AddCar(IRepository<Car> repository, CarPostRequest model)
        {
